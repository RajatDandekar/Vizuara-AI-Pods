...
function getStorageKey(slug: string, order: number) {
  return `${STORAGE_PREFIX}${slug}-${order}`;
}
...
export default function AssistantPageClient({
  ...
  const storageKey = getStorageKey(courseSlug, notebook.order);
      ...
      <div className="flex flex-col h-[calc(100vh-5rem)]">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-card-border bg-white">
          <div className="flex items-center gap-3 min-w-0">
            <Link
              href={`/courses/${courseSlug}/practice/${notebook.order}`}
