...
import { isCourseFullyComplete, issueCertificate, getCertificate } from '@/lib/progress';
...
export default function CertificatePageClient({
  ...
  useEffect(() => {
    ...
    // Gate: redirect if course not fully complete
    if (!isCourseFullyComplete(slug, notebookCount, hasCaseStudy)) {
      router.replace(`/courses/${slug}`);
      return;
    }

    // Check if certificate already exists (returning visitor)
    const existing = getCertificate(slug);
    const isFirstVisit = !existing;

    // Issue or retrieve certificate with the logged-in user's name
    const cert = issueCertificate(slug, courseTitle, difficulty, estimatedHours, notebookCount, user.fullName);
    setCertificate(cert);
  ...
  }, [slug, notebookCount, hasCaseStudy, courseTitle, difficulty, estimatedHours, router, user]);
  ...
  const handleDownload = useCallback(async () => {
    ...
    link.download = `vizuara-certificate-${slug}.png`;
