...
import type { NotebookMeta, CaseStudyMeta, CuratorInfo, CourseProgress } from '@/types/course';
...
export default function CourseOverviewClient({
  ...
  // Load progress on mount AND subscribe to changes from other pages
  useEffect(() => {
    setProgress(getProgress(slug));
    return onProgressChange(() => setProgress(getProgress(slug)));
  }, [slug]);
  ...
  // Which phase is currently expanded — auto-select based on progress
  ...
  // Continue CTA
  let continueHref = `/courses/${slug}/article`;
  let continueLabel = 'Start Learning';

  if (allContentComplete) {
    continueHref = `/courses/${slug}/certificate`;
    ...
    if (firstIncomplete) {
      continueHref = `/courses/${slug}/practice/${firstIncomplete.order}`;
      continueLabel = 'Continue Learning';
    } else if (caseStudy && !caseStudyDone) {
      continueHref = `/courses/${slug}/case-study`;
  ...
  // Determine which step the user should resume at
  const activeStep = (() => {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <FadeIn delay={0.3}>
        <div className="flex items-center gap-4 mb-10">
          ...
          {allContentComplete && (
            <Badge variant="green" size="md">Course Complete</Badge>
          )}
        </div>
      </FadeIn>

      {/* Course Content — Collapsible Phases */}
      <FadeIn delay={0.35}>
        <div className="space-y-3">
          ...
          >
            <Link href={`/courses/${slug}/article`}>
            ...
            >
              <div className="space-y-2">
                {notebooks.map((nb) => {
                  ...
                  return isAccessible ? (
                    <Link key={nb.slug} href={`/courses/${slug}/practice/${nb.order}`}>
                      {content}
                    </Link>
                  ) : (
          ...
          >
            {certificateStatus === 'locked' ? (
              <div className="text-center py-4 text-sm text-text-muted">
                ...
                Complete all course sections to unlock your certificate.
              </div>
            ) : (
